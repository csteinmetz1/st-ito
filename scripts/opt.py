...
def load_audio_feature_model():
    return None


def get_audio_feature_embeds(
    x: torch.Tensor,
...
def evaluate(
    ...
    # w = torch.sigmoid(w)
    w = torch.clamp(torch.from_numpy(w), min=0, max=1)
...
if __name__ == "__main__":
    ...
    input_filepath = "style-dataset/0phCNY-0Aiw.wav"
    target_filepath = "style-dataset/cLR1VwxSQrU.wav"
    ...
    w0 = np.ones(num_params) * 0.6
    sigma0 = 0.2

    es = cma.CMAEvolutionStrategy(w0, sigma0, {"bounds": [0, 1]})
    ...
    while not es.stop():
        ...
        if fopt < best_solution:
            best_solution = fopt
            torchaudio.save(
                f"sweep/{iteration}_output_audio.wav",
                output_audio,
                sr,
                backend="soundfile",
            )
